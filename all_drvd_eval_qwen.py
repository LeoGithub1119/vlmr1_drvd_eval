...
def build_prompt(sample: Dict[str, Any], task: str) -> Tuple[str, List[str]]:
    ...
    if task == "joint_qa":
        ...
        return jp2, list("ABCDEFGH")  # joint 允許 A-H（保守）
    else:
...
def qwen_vl_generate(processor, model, prompt: str, images: List[Image.Image], max_new_tokens: int) -> str:
    """
    關鍵修正：decode 只取「新生成 token」
    避免 prompt/options 內的 A./B./C. 汙染答案抽取。
    """
...
def main() -> int:
    ...
    ap.add_argument("--model-tag", default="model", help="Used in output filename (e.g., qwen3b, ov15)")
    ap.add_argument("--limit", type=int, default=0, help="0 = no limit")
    ap.add_argument("--max-new-tokens", type=int, default=64)  # joint 建議稍微拉高
        ...
        for i, sample in enumerate(tqdm(rows, total=len(rows))):
            try:
                ...
                if args.task == "joint_qa":
                    pred = extract_joint_letters(gen_text, k=4)
                else:
                    pred = extract_single_letter(gen_text, valid_letters if valid_letters else None)

                out_obj = dict(sample)
                out_obj["model_response"] = pred  # joint: "A,B,C,D" ; others: "A"
